...
class Env1():
    ...
    def getState(self, scan):
        ...
        print rel_theta, yaw
        if abs(h_theta) > abs(math.pi):
            h_theta = 2.0 * math.pi - abs(h_theta)
        else:
            h_theta = abs(h_theta)
        print ("human-UD angle: ", math.degrees(h_theta))
    ...
    def step(self):
        ...
        return state, rel_dis, yaw, diff_angle, diff_distance, done, arrive

    def cal_actor_pose(self, x,y , yaw,distance):
        xp = 0.
        yp = 0.
        rxp = 0.
        ryp = 0.
